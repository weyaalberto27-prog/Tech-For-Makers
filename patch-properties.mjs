...
content = content.replace(
  /updateElement\(comp\.id, \{ x: parseInt\(e\.target\.value\) \|\| 0 \}\)/g,
  `handleTransformChange(comp, { x: parseInt(e.target.value) || 0 })`
);
content = content.replace(
  /updateElement\(comp\.id, \{ y: parseInt\(e\.target\.value\) \|\| 0 \}\)/g,
  `handleTransformChange(comp, { y: parseInt(e.target.value) || 0 })`
);
content = content.replace(
  /updateElement\(comp\.id, \{\s*rotation: parseInt\(e\.target\.value\) \|\| 0,\s*\}\)/g,
  `handleTransformChange(comp, { rotation: parseInt(e.target.value) || 0 })`
);
content = content.replace(
  /updateElement\(comp\.id, \{\s*rotation: \(comp\.rotation \+ 90\) % 360,\s*\}\)/g,
  `handleTransformChange(comp, { rotation: (comp.rotation + 90) % 360 })`
);

fs.writeFileSync('src/components/PropertiesPanel.tsx', content);
console.log("Patched PropertiesPanel to support moving/rotating PCB components and their pads together");
